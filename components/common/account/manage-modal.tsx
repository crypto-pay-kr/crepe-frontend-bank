...
import { registerBankAccount } from "@/api/bankAccountApi"
...
export default function AccountRegistrationModal({ isOpen, onClose, onSubmit }: AccountRegistrationModalProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // API 호출
      await registerBankAccount(depositorName, currency, address, tag)
      ...
      console.error("계좌 등록 실패:", error.message)
      alert(`계좌 등록 실패: ${error.message}`)
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-black">
          <h2 className="text-xl font-medium text-pink-500">계좌 등록</h2>
        ...
        <div className="flex justify-center gap-4 p-6">
          <button
            ...
            className={`px-6 py-3 bg-blue-900 text-white font-medium rounded-md hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
